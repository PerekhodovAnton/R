...
```
1. t = -11.562 - разницу между средними значениями двух выборок (SE). Среднее первой выборки выше среднего второй.

df = 829.89 - степени свободы

Вывод: Можем отвергнуть нулевую гипотезу, т.к. p-value меньше 2,2e-16

2. t = -7.5116, df = 822.67, p-value = 1.523e-13

Вывод: Можем отвергнуть нулевую гипотезу.
```
...
```
T-test предполагает, что данные распределены нормально и нет выбросов.
Предлагается тест Шапиро-Уилка
```
...
```{r} 
ggplot(data = dur_word_freq, aes(x = LogDurationW, y = Cond)) +
  geom_boxplot(notch=TRUE) +
  theme_minimal()
```
```{r} 
ggplot(data = dur_word_freq, aes(x = LogDurationA, y = Cond)) +
  geom_boxplot(notch=TRUE)+ 
  theme_minimal()
```
...
wilcox_word <- wilcox.test(LogDurationW ~ Cond, data = dur_word_freq, paired = FALSE)
print(wilcox_word)

wilcox_word <- wilcox.test(LogDurationA ~ Cond, data = dur_word_freq, paired = FALSE)
print(wilcox_word)
